...
#     image = RTesseract.new(image_path)
#     expect(image.source).to eql(Pathname.new(image_path))
#   end

#   it ' select the language' do
#     # English
#     expect(RTesseract.new(image_path, lang: 'eng').lang).to eql(' -l eng ')
#     expect(RTesseract.new(image_path, lang: 'en').lang).to eql(' -l eng ')
#     expect(RTesseract.new(image_path, lang: 'en-US').lang).to eql(' -l eng ')
#     expect(RTesseract.new(image_path, lang: 'english').lang).to eql(' -l eng ')

#     # Portuguese
#     expect(RTesseract.new(image_path, lang: 'por').lang).to eql(' -l por ')
#     expect(RTesseract.new(image_path, lang: 'pt-BR').lang).to eql(' -l por ')
#     expect(RTesseract.new(image_path, lang: 'pt-br').lang).to eql(' -l por ')
#     expect(RTesseract.new(image_path, lang: 'pt').lang).to eql(' -l por ')
#     expect(RTesseract.new(image_path, lang: 'portuguese').lang).to eql(' -l por ')

#     expect(RTesseract.new(image_path, lang: 'eng').to_s_without_spaces).to eql('43XF')

#     expect(RTesseract.new(image_path, lang: 'eng').lang).to eql(' -l eng ')
#     expect(RTesseract.new(image_path, lang: 'it').lang).to eql(' -l ita ')

#     # Invalid lang object
#     expect(RTesseract.new(image_path, lang: MakeStringError.new).lang).to eql('')
#   end

#   it ' select options' do
#     expect(RTesseract.new(image_path).options_cmd).to eql([])
#     expect(RTesseract.new(image_path, options: 'digits').options_cmd).to eql(['digits'])
#     expect(RTesseract.new(image_path, options: :digits).options_cmd).to eql([:digits])
#     expect(RTesseract.new(image_path, options: [:digits, :quiet]).options_cmd).to eql([:digits, :quiet])
#   end

#   it ' support pdf output mode' do
#     # Internal test. Consider 'pdf' option only when #to_pdf is called.
#     expect(RTesseract.new(image_path, options: 'pdf').options_cmd).to eql([])
...
#     pdf_ocr = RTesseract.new(image_path)
#     expect(File.exists?(pdf_ocr.to_pdf)).to eql(true)
#     expect(File.extname(pdf_ocr.to_pdf)).to eql('.pdf')
...
#     expect(RTesseract.new(image_path, chop_enable: 0, enable_assoc: 0, display_text: 0).config).to eql("chop_enable 0\nenable_assoc 0\ndisplay_text 0")
#     expect(RTesseract.new(image_path, chop_enable: 0).config).to eql('chop_enable 0')
#     expect(RTesseract.new(image_path, chop_enable: 0, enable_assoc: 0).config).to eql("chop_enable 0\nenable_assoc 0")
#     expect(RTesseract.new(image_path, chop_enable: 0).to_s_without_spaces).to eql('43XF')
#     expect(RTesseract.new(image_path, tessedit_char_whitelist: 'ABCDEF12345').to_s_without_spaces).to eql('43F')
#   end

#   it ' crop image' do
#     expect(RTesseract.new(image_path, psm: 7).crop!(w: 36, h: 40, x: 140, y: 10).to_s_without_spaces).to eql('4')
#     expect(RTesseract.new(image_path, psm: 7).crop!(w: 36, h: 40, x: 180, y: 10).to_s_without_spaces).to eql('3')
#     expect(RTesseract.new(image_path, psm: 7).crop!(w: 20, h: 40, x: 216, y: 10).to_s_without_spaces).to eql('X')
#     expect(RTesseract.new(image_path, psm: 7).crop!(w: 30, h: 40, x: 240, y: 10).to_s_without_spaces).to eql('F')
#   end
...
#     expect(RTesseract.new(Magick::Image.read(image_path.to_s).first).to_s_without_spaces).to eql('43XF')
...
#     expect {
#       instance = RTesseract.read(image_path, processor: 'mini_magick') {}
...
#     expect(RTesseract.new(image_path).processor.a_name?('rmagick')).to eql(true)

#     # MiniMagick
#     RTesseract.configure { |config| config.processor = 'mini_magick' }
#     expect(RTesseract.new(image_path).processor.a_name?('mini_magick')).to eql(true)

#     # NoneMagick
#     RTesseract.configure { |config| config.processor = 'none' }
#     expect(RTesseract.new(image_path).processor.a_name?('none')).to eql(true)

#     # overwrite default
#     RTesseract.configure { |config| config.processor = 'rmagick' }
#     expect(RTesseract.new(image_path, processor: 'mini_magick').processor.a_name?('mini_magick')).to eql(true)

#     RTesseract.configure { |config| config.lang = 'portuguese' }
#     expect(RTesseract.new(image_path).lang).to eql(' -l por ')

#     RTesseract.configure { |config| config.psm = 7 }
#     expect(RTesseract.new(image_path).psm).to eql(' -psm 7 ')

#     RTesseract.configure { |config| config.tessdata_dir = '/tmp/test' }
#     expect(RTesseract.new(image_path).tessdata_dir).to eql(' --tessdata-dir /tmp/test ')

#     RTesseract.configure { |config| config.user_words = '/tmp/test' }
#     expect(RTesseract.new(image_path).user_words).to eql(' --user-words /tmp/test ')

#     RTesseract.configure { |config| config.user_patterns = '/tmp/test' }
#     expect(RTesseract.new(image_path).user_patterns).to eql(' --user-patterns /tmp/test ')
#   end
...
#     expect(RTesseract.new(image_path, tessdata_dir: '/tmp/test').tessdata_dir).to eql(' --tessdata-dir /tmp/test ')
#     expect(RTesseract.new(image_path, user_words: '/tmp/test').user_words).to eql(' --user-words /tmp/test ')
#     expect(RTesseract.new(image_path, user_patterns: '/tmp/test').user_patterns).to eql(' --user-patterns /tmp/test ')

#     expect(RTesseract.new(image_path, tessdata_dir: MakeStringError.new).tessdata_dir).to eql('')
